...
class spring(sf.RectangleShape):
	def __init__(self,width,height,posX,posY,color,oColor,oThick,mass,k=5):
		...
		self.numBounces=1
		self.curMaxHeight=self.posNot[1]+utlF.calcCurMaxHeight(self.springConstant,self.numBounces)
	...
	def bounce(self,floor):
		...
		#	cDelt=-self.deltas[len(self.deltas)-1]
			...
			self.curMaxHeight+=utlF.calcCurMaxHeight(self.springConstant,self.numBounces)
